...
internal class AddedCommentWithoutAttachmentsView(
  context: Context,
  attrs: AttributeSet?,
  ...
  override fun onAttachedToWindow() {
    ...
    retryButton.clicks
        ...
        .onEach { (key, files) -> presenter.sendEvent(RetryAttachmentsForComment(key, files)) }
        .launchIn(viewScope)
    goBackButton.setOnClickListener { activity.onBackPressed() }
  }

  private fun renderSubmitState(state: SubmitState) {
    if (state is FailedToAttachForComment) {
      addedCommentInfo.text = resources.getString(R.string.squash_it_commented, state.key.value)
    ...
    val isRetrying = state is RetryingAttachmentsForComment
